...
void AFG_PlayerCharacter::SetActive(bool IsActive)
{
	this->SetActorTickEnabled(IsActive);
}

void AFG_PlayerCharacter::PushAirborneState()
{
	LocomotionCoreState->SubStateMachine->Push(LocomotionCoreState->LocomotionAirborneSubState);
}

void AFG_PlayerCharacter::AllowAnotherDoubleJump()
{
	LocomotionCoreState->SubStateMachine->Remove(LocomotionCoreState->LocomotionAirJumpSubState);
}
