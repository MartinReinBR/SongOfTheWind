...
void UFG_InputWriteComponent::BlockInput(float Time)
{
	FTimerManager& TimerManager = GetOwner()->GetWorldTimerManager();
	InputStats->BlockInput(TimerManager, Time);
}

bool UFG_InputWriteComponent::IsBlocked() const
{
	return InputStats->IsBlocked();
}
