...
void UFG_CollectibleComponent::BeginPlay()
{
	...
	FVector RayDown = GetOwner()->GetActorLocation() - FVector(0, 0, FloatingDistanceFromGround * 100.0f);
	FCollisionQueryParams CollisionParams;
	CollisionParams.AddIgnoredActor(GetOwner());
	FHitResult Result;
	if (GetWorld()->LineTraceSingleByChannel(Result, GetOwner()->GetActorLocation(), RayDown, ECC_WorldStatic, CollisionParams))
	{
		GetOwner()->SetActorLocation(Result.Location + FVector{0.0f, 0.0f, FloatingDistanceFromGround});
	}
	// ...
...
void UFG_CollectibleComponent::SetIsTickable(bool IsTickable) const
{
	AActor* Self = GetOwner();
	Self->SetActorTickEnabled(IsTickable);
}

void UFG_CollectibleComponent::ShowSelf() const
{
	AActor* Self = GetOwner();
